                      ...
                      {chapter.videos.map((v, vi) => {
                        ...
                        return (
                          <li key={v.youtube_id} className="flex items-center gap-3 p-2.5 rounded-2xl bg-white border-2 border-[color:var(--card-line)] hover:border-[#D7DCEF]">
                            <span className="relative shrink-0 w-16 h-10 rounded-xl overflow-hidden bg-[#F1F3FB]">
                              <img src={youtubeThumbnail(v.youtube_id)} alt="" loading="lazy" className="w-full h-full object-cover" />
                              <span
                                className={`absolute left-1 top-1 w-[18px] h-[18px] rounded-full flex items-center justify-center text-[9px] font-extrabold ${
                                  completed ? 'bg-[#12A594] text-white' : 'bg-black/55 text-white'
                                }`}
                                aria-label={completed ? 'Completed' : 'Not started'}
                              >
                                {completed ? <Check className="w-3 h-3" strokeWidth={3.5} /> : vi + 1}
                              </span>
                            </span>
